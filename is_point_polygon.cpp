...
/*  Given a polygon and a point ‘p’, find if ‘p’ lies inside the polygon or not.
    ...
    3) A point is inside the polygon if either count of intersections is odd or
       point lies on an edge of polygon.  If none of the conditions is true, then point lies outside.*/
...
bool isInside(Point polygon[], int n, Point p) {
    ...
    do {
        ...
        if(isIntersection(polygon[i], polygon[next], p, extreme)) {
...
int main() {
    ...
    */
    ...
    for(int i = 0; i < n; i++) scanf("%d %d", &polygon[i].x, &polygon[i].y);
    scanf("%d %d", &p.x, &p.y);

    if(isInside(polygon, n, p)) printf("Yes\n");
    else printf("No\n");
    return 0;
}
